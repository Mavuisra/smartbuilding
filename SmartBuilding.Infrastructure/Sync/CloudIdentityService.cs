...
public sealed class CloudIdentityService
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        var existingUserToken = await CloudApiAuth.TryLoginAsync(baseUrl, username, password, cancellationToken)
            .ConfigureAwait(false);
        if (!string.IsNullOrWhiteSpace(existingUserToken))
        {
            SyncCloudTokenStore.Persist(existingUserToken);
            return CloudIdentityResult.Ok(
                existingUserToken,
                $"Compte « {username} » connecté au cloud — synchronisation initiale des données.");
        }
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        await _organizationCloudSync.RegisterActiveOrganizationAsync(cancellationToken: cancellationToken)
            .ConfigureAwait(false);
        ...
        var activeOrg = _organizationRegistry.Active;
        if (activeOrg is not null)
            api.SetOrganizationId(activeOrg.Id.ToString());
